	...
	var Json = my.Class({
		...
		STATIC:{
			...
			find: function(obj,val,para){
				...
				if(obj.constructor == Array){
					arrayRecursive(obj,-1);
				}else if(obj.constructor == Object){
					objectRecursive(obj,-1);
				}else{
					return false;
				}
				...
				//scan object
				function objectRecursive(obj,level){
					for(q in obj){
						dec(obj,q,level);
					}
				...
				//scan array
				function arrayRecursive(ary,level){
					if(ary.length > 0){
						for(var i=0; i<ary.length; i++){
							dec(ary,i,level);
						}
